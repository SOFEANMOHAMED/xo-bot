...
-- Official XO Bot page content publishing (platform-scoped — no merchant_id)
...
CREATE INDEX IF NOT EXISTS idx_platform_content_publications_page ON platform_content_publications(page_id);
CREATE INDEX IF NOT EXISTS idx_platform_content_publications_status ON platform_content_publications(status);
CREATE INDEX IF NOT EXISTS idx_platform_content_publications_due ON platform_content_publications(scheduled_at) WHERE status = 'scheduled';
...
CREATE INDEX IF NOT EXISTS idx_platform_content_publication_media_pub ON platform_content_publication_media(publication_id);
...
CREATE TABLE IF NOT EXISTS platform_content_publication_targets (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT uq_platform_content_publication_target UNIQUE (publication_id, platform, account_ref)
);

CREATE INDEX IF NOT EXISTS idx_platform_content_publication_targets_pub ON platform_content_publication_targets(publication_id);
CREATE INDEX IF NOT EXISTS idx_platform_content_publication_targets_status ON platform_content_publication_targets(status);

-- Per-merchant LLM token ledger (GPT-4o mini). NULL merchant_id = platform-level usage.
