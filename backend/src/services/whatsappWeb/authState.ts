...
import { decryptSessionBlob, encryptSessionBlob } from './crypto.js';
import {
  getWhatsAppWebSession,
  saveEncryptedAuthState,
...
export async function createPostgresAuthState(merchantId: string): Promise<{
  ...
  if (row?.creds_ciphertext && row?.keys_ciphertext) {
    try {
      ...
      logger.error('Failed to decrypt WhatsApp session — starting a fresh pairing', error as Error, {
        merchantId
      });
      creds = initAuthCreds();
      keys = {};
  ...
  const persistNow = async () => {
    ...
    await saveEncryptedAuthState(merchantId, credsCiphertext, keysCiphertext);
  };
  ...
  const schedulePersist = () => {
    ...
    persistTimer = setTimeout(() => {
      ...
      persistChain = persistChain.then(persistNow).catch((error) => {
        logger.error('Failed to persist WhatsApp auth state', error as Error, { merchantId });
      });
    }, 250);
  };
