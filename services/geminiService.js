...
3. CURRENCY: Always use "{{storeCurrency}}" for prices. Do not convert unless explicitly asked.
 ...
 */
export const generateStoreResponse = async (chatHistory, products, baseSystemPrompt, storeName, storeCurrency, context = 'web', persona = 'friendly', policies) => {
    if (!API_KEY) {
        return "عذراً، لم يتم ضبط مفتاح API الخاص بالذكاء الاصطناعي. يرجى التحقق من الإعدادات.";
    }
    // Get base URL for image endpoint
    const baseUrl = import.meta.env?.VITE_API_URL?.replace('/api', '') || 'https://xo-bot.com';
    ...
    const productContext = products.map(p => {
        ...
        return `- ID: ${p.id}
     - Name/الاسم: ${p.name}
     - Price/السعر: ${p.price} (Currency: ${storeCurrency})
 ...
 */
export const generateServiceResponse = async (chatHistory, services, storeName) => {
    if (!API_KEY) {
        return "عذراً، لم يتم ضبط مفتاح API.";
    }
    const servicesContext = services.map(s => `Service ID: ${s.id}
     Name: ${s.name}
     ...
     Booking: ${s.bookingLink || 'N/A'}`).join('\n\n');
 ...
 */
export const generateProductDescription = async (productName, keywords, category, imageBase64) => {
    if (!API_KEY)
        return null;
 ...
 */
export const generateSaaSAssistantResponse = async (query, botType) => {
    if (!API_KEY)
 ...
 */
export const generateMarketingImage = async (prompt, aspectRatio = '1:1', imageSize = '1K', referenceImageBase64) => {
    if (!API_KEY)
        return null;
    try {
