...
const AdminEmailBroadcast: React.FC = () => {
  const [formData, setFormData] = useState<EmailBroadcastRequest>({
    subject: '',
    message: '',
    recipientType: 'all',
    customEmails: [],
  ...
  useEffect(() => {
    const fetchRecipientCount = async () => {
      ...
      try {
        const count = await apiService.getEmailRecipientCount(formData.recipientType);
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.recipientType === 'custom') {
      if (!formData.customEmails || formData.customEmails.length === 0) {
        newErrors.customEmails = 'يجب إضافة عنوان بريد إلكتروني واحد على الأقل';
      } else {
        const invalidEmails = formData.customEmails.filter(email => !validateEmail(email));
        if (invalidEmails.length > 0) {
          newErrors.customEmails = `عناوين بريد إلكتروني غير صحيحة: ${invalidEmails.join(', ')}`;
        }
      }
    }
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await apiService.sendEmailBroadcast(formData);
      setSendResult(result);
      
      if (result.success && result.sent > 0) {
        showSuccess(`تم إرسال ${result.sent} بريد إلكتروني بنجاح`);
        // Reset form
        setFormData({
          subject: '',
          message: '',
        ...
        showError(`فشل إرسال ${result.failed} بريد إلكتروني`);
      }
    } catch (error) {
      const errorMessage = handleApiError(error);
      showError(errorMessage || 'حدث خطأ أثناء إرسال البريد الإلكتروني');
      setSendResult({
        success: false,
        sent: 0,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">إرسال بريد إلكتروني جماعي</h2>
            <p className="text-slate-400 text-sm">أرسل رسائل بريد إلكتروني إلى المشتركين</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              ...
              { value: 'custom', label: 'قائمة مخصصة', icon: Mail }
        ...
        {formData.recipientType === 'custom' && (
          <div>
            ...
            {errors.customEmails && (
              <p className="text-red-400 text-sm mb-2">{errors.customEmails}</p>
            )}
            {formData.customEmails && formData.customEmails.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {formData.customEmails.map((email) => (
                  <div
                    key={email}
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            يمكنك استخدام HTML في الرسالة (سيتم إرسالها كـ HTML)
          </p>
        </div>
